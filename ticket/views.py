...
class DashboardView(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        all_tasks = Ticket.objects.filter(assigned_user=user,
                                          accepted=True).count()
        open_tasks = Ticket.objects.filter(assigned_user=user,
                                           accepted=True, state="open").count()

        if all_tasks == 0 or open_tasks == 0:
            tasks = 0
        else:
            tasks = 100 - round(100 * (open_tasks / all_tasks))
...
def rec_cron(request):
    ...
    for ticket in tickets:
        ...
        occ = ticket.recurrences.after(datetime.today().replace(microsecond=0),
                                       inc=True)
